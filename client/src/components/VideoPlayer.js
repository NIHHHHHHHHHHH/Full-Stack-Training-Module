...
const VideoPlayer = ({ video, watchedTime, setWatchedTime, setIsVideoCompleted }) => {
  ...
  useEffect(() => {
    ...
    if (video?.url) { // Only proceed if video.url is available
      // Use a timeout to ensure the DOM element is fully mounted
      ...
      }, 0); // A timeout of 0ms puts it at the end of the current event loop
